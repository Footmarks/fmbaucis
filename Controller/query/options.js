...
var decorator = module.exports = function () {
  ...
  // Apply incoming request page.
  this.query(function (request, response, next) {
    var page = request.query.page;
    if (page === undefined || page === null) return next();
    if (!isNonNegativeInteger(page)) {
      return next(BaucisError.BadRequest('page must be a non-negative integer if set'));
    }
    request.baucis.query.skip(page * request.query.per_page);
    next();
  });
  // Apply incoming request limit.
  this.query(function (request, response, next) {
    var per_page = request.query.per_page;
    if (per_page === undefined || per_page === null) return next();
    if (!isPositiveInteger(per_page)) {
      return next(BaucisError.BadRequest('per_page must be a positive integer if set'));
    }
    request.baucis.query.limit(per_page);
    next();
  });
  // Set count flag.
  this.query(function (request, response, next) {
